...
if rem(Row,2)==0
    for u=-Row/2:Row/2-1
        if rem(Column,2)==0
            for v=-Column/2:Column/2-1
                H(u+Row/2+1,v+Column/2+1)=exp(-0.001*(u^2+v^2)^(5/6));
            end
        else 
            for v=-(Column-1)/2:(Column-1)/2
                H(u+Row/2+1,v+(Column-1)/2+1)=exp(-0.001*(u^2+v^2)^(5/6));
            end
        end
    end
else
    for u=-(Row-1)/2:(Row-1)/2
        if rem(Column,2)==0
            for v=-Column/2:Column/2-1
                H(u+(Row-1)/2+1,v+Column/2+1)=exp(-0.001*(u^2+v^2)^(5/6));
            end
        else
            for v=-(Column-1)/2:(Column-1)/2
                H(u+(Row-1)/2+1,v+(Column-1)/2+1)=exp(-0.001*(u^2+v^2)^(5/6));
            end
        end
    end
end
...
figure(2);
imshow(Image_Deg,[]);  
Image_fft=fftshift(fft2(Image_Deg));    %傅里叶变换,平移
...
result=deconvwnr(Image_Deg,h_estimate,0.001);
